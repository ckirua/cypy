...
| `cybytes` | `bytes` | public + cimport | `bytes_len`, `bytes_contains`, `bytes_eq`, `bytes_ne`, `bytes_startswith`, `bytes_endswith`; `bytes_as_string` cimport-only |
...
|--------|---------|--------|
| `cydatetime`, `cycodecs`, `cymarshal`, `cyfileobject`, `cyweakref`, `cypycapsule`, `cycontextvars` | public (+ some cimport) | Higher-level C-API bridges; `dt_date_eq` / `dt_time_eq` / `dt_datetime_eq` / `dt_timedelta_eq` / `capsule_eq` / `weakref_eq` on `cypy` (not `hot`) |
...
| `cyfunction`, `cymethod`, `cymodule`, `cyiterator`, `cyiterobject`, `cygenobject`, `cycellobject`, `cydescr`, `cytype`, `cylong`, `cyfloat`, … | public | Object-model / scalar helpers; `type_eq` identity / `cell_eq` content (not `hot`) |
